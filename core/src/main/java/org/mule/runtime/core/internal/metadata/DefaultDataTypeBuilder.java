...
public class DefaultDataTypeBuilder
  ...
  private static Cache<Class, ProxyIndicator> proxyClassCache = Caffeine.newBuilder().weakKeys().weakValues().build();
  private static Cache<Class, ProxyIndicator> cglibClassCache = Caffeine.newBuilder().weakKeys().weakValues().build();

  // This cannot be changed to Caffeine because it live-locks when generating type hierarchies (collections, maps, functions)
  // Ref: https://github.com/ben-manes/caffeine/issues/209
  ...
  protected static <T> boolean isProxyClass(Class<T> type) {
    ProxyIndicator indicator = proxyClassCache.get(type, t -> {
      // Either there's no indicator in the map or there's one that is due to be replaced
      return new ProxyIndicator(t, Proxy.isProxyClass(t));
    });

    if (indicator.getTargetClass() == type) {
  ...
  protected static <T> boolean isCglibClass(Class<T> type) {
    ProxyIndicator indicator = cglibClassCache.get(type, t -> {
      // Either there's no indicator in the map or there's one that is due to be replaced
      ...
      return new ProxyIndicator(t, t.getName().contains("CGLIB$$"));
    });
  ...
  public boolean equals(Object obj) {
    ...
    return Objects.equals(typeRef.get(), other.typeRef.get())
        && Objects.equals(itemTypeBuilder, other.itemTypeBuilder)
        && Objects.equals(keyTypeBuilder, other.keyTypeBuilder)
        && Objects.equals(valueTypeBuilder, other.valueTypeBuilder)
        && Objects.equals(returnType, other.returnType)
        && Objects.equals(parametersType, other.parametersType)
